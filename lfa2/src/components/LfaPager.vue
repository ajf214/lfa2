<template>
  <div class="pager">
      <button :class="{selected: currentPage === index}" v-on:click="changePage(index)" v-for="index in pages" :key="index">{{index}}</button>
  </div>
</template>
...
export default {
    ...
    methods: {
        getItemCount: async function () {
            ...
            const response = await axios.get(`${this.API_ENDPOINT}/all-items?unsold=${this.unsold}`)
